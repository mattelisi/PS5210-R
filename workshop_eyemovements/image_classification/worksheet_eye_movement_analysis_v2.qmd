...
(note we use `vapply()` to extract scalar values from `ds2$trial`, which is a list type of object)
...
plot(tr$timestamp, XY[, 1], type = "l", lwd = 2, col = "red",
     xlab = "Time from image onset [ms]", ylab = "Position [px]", ylim=range(XY))
...
**Meaning of output variables from `saccpar()`**

- `a`      : Start sample index of the saccade (first threshold-crossing sample).
- `b`      : End sample index of the saccade (last sample belonging to the event).
- `d`      : Duration in samples $(b - a + 1)$. Convert to ms via d / sampling_rate * 1000.
- `vpeak`  : Peak velocity magnitude ($\max \sqrt{v_x^2 + v_y^2}$) within the saccade.
- `dist`   : Straight-line displacement magnitude between start and end positions: $\sqrt{(x_b - x_a)^2 + (y_b - y_a)^2}$.
- `angd`   : Direction of straight-line displacement (radians), computed with $\text{atan2}(d_y, d_x)$.
- `ampl`   : Trajectory amplitude based on signed position ranges within the saccade; reflects full movement excursion and may exceed dist for curved paths.
- `anga`   : Direction of trajectory range vector (radians), computed with $\text{atan2}(\text{range}_y, \text{range}_x)$.
...
- [ ] Re-run the saccade and fixation analysis for different participants and trials.
