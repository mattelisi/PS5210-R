...
for (i in seq_len(nrow(msg))) {
  ...
  if (sa[1] == "TRIAL_END" && length(sa) >= 2) {
    ...
    t_end <- as.integer(msg$time[i]) + 100 # add extra ms for visualisation
  }
...
```{r}
cat("Number of parsed trials:", nrow(trial_tab), "\n")

cat("Overall accuracy:\n")
mean(trial_tab$correct, na.rm = TRUE)

cat("Accuracy by trial type (anti_saccade: 0 = pro, 1 = anti):\n")
